...
import { Card, DataTable, StatusChip, Spinner, RiskChip } from '../../ui';
  ...
  return (
    <>
      ...
      <div className="filters mb16">
        ...
        <input placeholder={`${t('search')} (нэр, регистр, vendor №)`} value={search}
      ...
      {!rows ? <Spinner /> : (
        <Card tight>
          <DataTable rows={rows} onRow={r => nav(`/admin/suppliers/${r.id}`)} cols={[
            { key: 'id', label: 'ID', w: 64 },
            { key: 'name_mn', label: t('name'), w: 260, wrap: true, render: r => <><div className="bold">{r.name_mn}</div><div className="mut">{r.name_en}</div></> },
            ...
            { key: 'risk_level', label: t('risk'), render: r => <RiskChip r={r.risk_level} /> },
            { key: 'category_codes', label: t('categories'), w: 190, wrap: true, render: r => r.category_codes ? <span className="mut">{r.category_codes}</span> : '—' },
            ...
            { key: 'primary_contact', label: lang === 'mn' ? 'Холбоо барих' : 'Contact', w: 200, wrap: true },
          ]} />
        </Card>
      )}
    </>
